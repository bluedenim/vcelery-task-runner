        ...
        try:
            if task_name:
                ...
                for task_param in task_params:
                    ...
                    if posted_value:
                        if issubclass(task_param.annotation, datetime):
                            param_value = datetime.fromisoformat(posted_value)
                        elif issubclass(task_param.annotation, BaseModel):
                            param_value = task_param.annotation.model_validate_json(posted_value)
                        elif task_param.annotation == Parameter.empty:
                            logger.warning(f"No type hint available for {task_param.name}. Using str.")
                            param_value = str(posted_value)
                        else:
                            param_value = task_param.annotation(posted_value)
                    else:
                        if not has_default:
                            # Missing parameter
                            raise ValueError(f"Missing value for {task_param.name}")
                        else:
                            param_value = task_param.default.value
